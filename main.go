...
func main() {
	...
	for _, structName := range structNames {
		...
		if err := generateStructFromFields(pkgName, pkgPath, out, structName, fields); err != nil {
			log.Fatalln(err)
		}
...
func generateStructFromFields(pkgName, pkgPath, out, structName string, fields []StructField) error {
...
func generateWither(f *jen.File, pkgPath, structName string, field StructField) {
	// Output:
	// WithName sets name.
	...
	f.Func().Params(
		Id(shortName).Id(builderName), // (b *FooBuilder)
			...
			Params(
			// name string
			Id(lowerFirst(field.Name)).Qual(skipCurrentPackagePath(pkgPath, field), field.FieldType),
		).
...
func generateWitherPointer(f *jen.File, pkgPath, structName string, field StructField) {
	// Output:
	// WithName sets name.
